  ...
  return (
    <div className="flex flex-col gap-6 sticky top-6">
      <div className="card overflow-hidden">
        <div className="sidebar-card-header border-b border-dark-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-red animate-pulse" />
            <div className="w-2 h-2 rounded-full bg-amber" />
            <div className="w-2 h-2 rounded-full bg-green" />
            <span className="sidebar-card-title">SYSTEM LOG</span>
          </div>
          {onClearLogs && (
            <button
              onClick={onClearLogs}
      ...
      <div className="card overflow-hidden">
        <div className="sidebar-card-header border-b border-dark-border flex justify-between items-center">
          <p className="sidebar-card-title">SCAN HISTORY</p>
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted">{history.length} saved</span>
        ...
        <div className="max-h-80 overflow-y-auto scrollbar-thin">
          {history.length === 0 ? (
            <p className="text-xs text-dark-muted p-8 text-center italic">No scans yet</p>
          ) : (
            history.map((item) => (
              <div
                key={item.id}
                className="p-3 border-b border-dark-border flex gap-3 items-start hover:bg-surface2 transition-colors group"
              >
                {item.preview && (
                  <img
                    src={item.preview}
                    alt=""
                    className="w-12 h-12 object-cover rounded border border-dark-border shrink-0 group-hover:border-green/30 transition-colors"
                    loading="lazy"
                  />
                )}
